...
With the code above, we are able to display the correlation between variables. As we can see from the figure, `attitude` and `points` have the most significant correlation. I suppose this makes sense in general.  `deep` and `surf` for some reason also has a strong correlation but it is a negative correlation.
...
The assumption of the simple linear model is that all explanatory variables follow a linear pattern with the target variable.
