...
The plot above shows how well the random intercept and slope model with interaction fits. It is displayed side by side, where the plot of data is on the left side and the fitted model is placed on the right side. The `treatment` is visualized by color and the `subject` is visualized by lifestyle. 
